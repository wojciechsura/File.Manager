...
namespace File.Manager.Geometry
{
		...
		where TConcretePoint : BaseIntPoint<TConcretePoint>
    {
        ...
        public TConcreteRectangle Offset(int dLeft, int dTop, int dRight, int dBottom) 
        {
            ...
            int newTop = Top + dTop;
            int newRight = Math.Max(newLeft, Right + dRight);
            int newBottom = Math.Max(newTop, Bottom + dBottom);

            return CreateRectangle(newLeft, newTop, newRight, newBottom);
        }
		...
		where TConcretePoint : BaseFloatPoint<TConcretePoint>
    {
        ...
        public TConcreteRectangle Inflate(float dLeft, float dTop, float dRight, float dBottom) 
        {
            float newLeft = Left + dLeft;
            float newTop = Top + dTop;
            float newRight = Math.Max(newLeft, Right + dRight);
            float newBottom = Math.Max(newTop, Bottom + dBottom);
		...
		where TConcretePoint : BaseDoublePoint<TConcretePoint>
    {
        ...
        public TConcreteRectangle Inflate(double dLeft, double dTop, double dRight, double dBottom) 
        {
            double newLeft = Left + dLeft;
            double newTop = Top + dTop;
            double newRight = Math.Max(newLeft, Right + dRight);
            double newBottom = Math.Max(newTop, Bottom + dBottom);
